...
namespace OpenRevitOleStorage.StructuredStorage
{
    ...
    public class BasicFileInfo : StorageStreamBase
    {
        ...
        private string _projectSparkFile = string.Empty;
        private string _centralModelidEntity = string.Empty;
        private string _localeWhenSaved = string.Empty;
        private string _allLocalChangesSavedToCentral = string.Empty;
        private string _uniqueDocumentGUID = string.Empty;
        private string _uniqueDocumentIncrements = string.Empty;
        private string _modelIdentity = string.Empty;
        ...
        public string ProjectSparkFile
        {
            get
            {
                return _projectSparkFile;
            }
            private set
            {
                _projectSparkFile = value;
            }
        }

        public string CentralModelidEntity
        {
            get
            {
                return _centralModelidEntity;
            }
            private set
            {
                _centralModelidEntity = value;
            }
        }

        public string LocaleWhenSaved
        {
            get
            {
                return _localeWhenSaved;
            }
            private set
            {
                _localeWhenSaved = value;
            }
        }

        public string AllLocalChangesSavedToCentral
        {
            get
            {
                return _allLocalChangesSavedToCentral;
            }
            private set
            {
                _allLocalChangesSavedToCentral = value;
            }
        }

        public string UniqueDocumentGUID
        {
            get
            {
                return _uniqueDocumentGUID;
            }
            private set
            {
                _uniqueDocumentGUID = value;
            }
        }

        public string UniqueDocumentIncrements
        {
            get
            {
                return _uniqueDocumentIncrements;
            }
            private set
            {
                _uniqueDocumentIncrements = value;
            }
        }
        ...
        private void ParseDetailInfo(string detailInfo)
        {
            ...
            switch (detailKey)
            {
                ...
                case "PROJECTSPARKFILE":
                    ProjectSparkFile = detailValue.Trim();
                    break;

                case "CENTRALMODELIDENTITY":
                    CentralModelidEntity = detailValue.Trim();
                    break;

                case "LOCALEWHENSAVED":
                    LocaleWhenSaved = detailValue.Trim();
                    break;

                case "ALLLOCALCHANGESSAVEDTOCENTRAL":
                    AllLocalChangesSavedToCentral = detailValue.Trim();
                    break;

                //case "CENTRALMODEL'SEPISODEGUIDCORRESPONDINGTOTHELASTRELOADLATEST":
                // centralmodelsepisodeguidcorrespondingtothelastreloadlatest = Convert.ToInt32(detailValue.Trim());
                // break;

                case "UNIQUEDOCUMENTGUID":
                    UniqueDocumentGUID = detailValue.Trim();
                    break;

                case "UNIQUEDOCUMENTINCREMENTS":
                    UniqueDocumentIncrements = detailValue.Trim();
                    break;

                case "MODELIDENTITY":
                    ModelIdentity = detailValue.Trim();
                    break;
        ...
        public override string ToString()
        {
            ...
            try
            {
                if (this != null)
                {
                    ...
                    sb.Append(string.Format("ProjectSparkFile: <{0}>{1}", ProjectSparkFile, Environment.NewLine));
                    sb.Append(string.Format("CentralModelidEntity: <{0}>{1}", CentralModelidEntity, Environment.NewLine));
                    sb.Append(string.Format("LocaleWhenSaved: <{0}>{1}", LocaleWhenSaved, Environment.NewLine));
                    sb.Append(string.Format("AllLocalChangesSavedToCentral: <{0}>{1}", AllLocalChangesSavedToCentral, Environment.NewLine));
                    sb.Append(string.Format("UniqueDocumentGUID: <{0}>{1}", UniqueDocumentGUID, Environment.NewLine));
                    sb.Append(string.Format("UniqueDocumentIncrements: <{0}>{1}", UniqueDocumentIncrements, Environment.NewLine));
                    sb.Append(string.Format("ModelIdentity: <{0}>{1}", ModelIdentity, Environment.NewLine));
                    sb.Append(seperator);
